function item_blink_datadriven_on_spell_start(keys)
	...
	local player = keys.caster:GetPlayerID()
		local daun = ParticleManager:CreateParticle("particles/econ/events/ti9/blink_dagger_ti9_start.vpcf", PATTACH_WORLDORIGIN, keys.caster)
		ParticleManager:SetParticleControl(daun, 0, keys.caster:GetAbsOrigin())
		ParticleManager:SetParticleControl(daun, 1, Vector(100, 100, 0))
