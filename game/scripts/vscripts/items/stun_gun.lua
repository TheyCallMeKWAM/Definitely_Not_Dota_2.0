...
self.attackspeed = self.ability:GetSpecialValueFor("bonus_attack_speed")
self.strength = self.ability:GetSpecialValueFor("bonus_strength")
self.intellect = self.ability:GetSpecialValueFor("bonus_int")
...
function modifier_item_stun_gun:OnAttackLanded( keys )
	if IsServer() then
		...
		if chance >= RandomInt(1, 100) then
			LaunchLightning(attacker, target, ability, 200, 900)
		end
...
function modifier_item_stun_gun:GetModifierPreAttack_CriticalStrike(params)
	...
	if self:GetAbility():IsCooldownReady() and not self:GetCaster():IsRangedAttacker() then
		chance = 100
		print("critical 100%")
	end
	if chance >= RandomInt(1, 100) then
